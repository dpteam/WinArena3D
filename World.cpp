...
bool CwaWorld::CheckHitCritterMelee(DWORD &id)
{
	...
	int const c_WeaponReach = c_FixedPointOne; // DPT: added int
		...
		for (;;) {
			...
			if ((pivotRight - pivotLeft) < 7) {
				for (long tempRight = pivotLeft + 1; tempRight <= pivotRight; ++tempRight) {
					...
					//m_QuadSortList[tempLeft + 1].Depth = tempDepth; 					// DPT: Need to fix this.
					//m_QuadSortList[tempLeft + 1].Index = tempIndex;					// DPT: Need to fix this.
				}
